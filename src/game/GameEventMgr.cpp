...
void GameEventMgr::SaveWorldEventStateToDB(uint16 event_id)
{
    ...
    CharacterDatabase.PExecute("DELETE FROM game_event_save WHERE event_id = '%u'",event_id);
    if(mGameEvent[event_id].nextstart)
        CharacterDatabase.PExecute("INSERT INTO game_event_save (event_id, state, next_start) VALUES ('%u','%u',FROM_UNIXTIME('"UI64FMTD"'))",event_id,mGameEvent[event_id].state,mGameEvent[event_id].nextstart);
    else
